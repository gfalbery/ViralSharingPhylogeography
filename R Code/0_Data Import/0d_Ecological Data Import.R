...
DietComp <- EltonTraits %>% dplyr::select(starts_with("Diet")) %>% 
  dplyr::select(1:10) %>% as_tibble
Remove <- which(rowSums(DietComp)==0)
DietComp <- DietComp %>% slice(-Remove)

VD <- vegdist(DietComp) %>% as.matrix

colnames(VD) <- rownames(VD) <- EltonTraits %>% 
  slice(-Remove) %>% dplyr::select(Scientific) %>% unlist
