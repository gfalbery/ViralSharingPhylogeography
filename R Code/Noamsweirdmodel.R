...
DataList$VirusBinary <- DataList$VirusBinary %>%
  mutate(Gz = ifelse(Space == 0, 1, 0))

Resps <- c("VirusBinary","RNA","DNA","Vector","NVector")[1]

Covar <- c("s(Phylo, by = ordered(Gz))","s(Phylo, by = ordered(!Gz))",
           "s(Space)", "Gz",
           "ti(Phylo, Space, by = ordered(!Gz), k = 20)", 
           
           #"s(Phylo, by = ordered(Gz)) + t2(Phylo, Space, by = ordered(!Gz), k = 30)",
...
WeirdModel4 <- bam(Formula,
                   data = DataList[[Resps[r]]], 
                   family = binomial(),
                   paraPen = PPList[[Resps[r]]], select = T
)

save(WeirdModel4, file = "Output Files/WeirdModel4.Rdata")


stop()

load("Output Files/FitList.Rdata")
load("Output Files/Finaldf.Rdata")
         ...
         PhyloQuantile = ifelse(Phylo == last(unique(Phylo)), "0.1",
                                ifelse(Phylo == 0, "0",
                                       ifelse(Phylo == 0.25, "0.25",
                                              ifelse(Phylo == 0.5, "0.5", NA))))) %>% mutate(Gz = ifelse(Space>0,0,1))
