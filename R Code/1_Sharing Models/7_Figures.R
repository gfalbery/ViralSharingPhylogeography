...
Panth1 %>% group_by(hOrder) %>%
  summarise(Mean = mean(AllPredDegree),
            SD = sd(AllPredDegree),
            N = n()) %>%
  mutate(SE = SD/(N^0.5), Lower = Mean - SE, Upper = Mean + SE) %>%
  mutate(Upper = ifelse(is.na(Upper), Mean, Upper),
         Lower = ifelse(is.na(Lower), Mean, Lower)) %>%
  slice(order(Mean)) %>% 
