# 3d_High Resolution Spatial Degree Figure ####
...
rownames(Panth1) <- Panth1$Sp

AllPredVector <- Panth1[colnames(FullValuedf),"AllPredDegree"]

RemoveRowVector <- which(rowSums(FullValuedf, na.rm = T)==0)

FullValuedf <- FullValuedf %>% slice(-RemoveRowVector)

GridVector <- rep(NA, length = nrow(FullValuedf))

for(i in 1:nrow(FullValuedf)){
  
  if( i %% 1000 == 1) print(i)
  
  WhichRow <- which(!is.na(FullValuedf[i,]))
  
  if(length(WhichRow)>0)  GridVector[i] <- mean(AllPredVector[WhichRow], na.rm = T)
  
}

save(GridVector, RemoveRowVector, file = "GridVector.Rdata")
