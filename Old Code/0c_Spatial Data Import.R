...
hp3 = droplevels(Hosts %>%
  filter(hDom == 'wild', hMarOTerr == 'Terrestrial') %>%
  dplyr::select(Sp) %>%
  mutate(hp3 = 1))

hp3$Sp <- as.character(hp3$Sp)

spatial_host <- terr

# Join spatial polygons and hp3 data frames
spatial_host@data = left_join(spatial_host@data, hp3[hp3$Sp%in%spatial_host@data$BINOMIAL,], by = c("BINOMIAL" = 'Sp')) %>%
  subset(hp3 == 1)

#output shape file for QGIS
writeOGR(obj=spatial_host, dsn=P("maps/"), driver = "ESRI Shapefile", layer = "hosts")
writeOGR(obj=spatial_host, dsn=P("maps/"), driver = "ESRI Shapefile", layer = "hp3")
...
hostdf<-data.frame(host_@data)
hostdf<-data.frame(spatial_host@data)
...
iucnhostnames <- spatial_host$BINOMIAL
